(function () {
  ...
  function plotGraph(data) {
    ...
    const maxTemp = Math.max(
      ...data.ant,
      ...data.hadData.val.map((v, i) => v + data.hadData.error_upper[i] || 0),
    );
    const minTemp = Math.min(
      ...data.ant,
      ...data.hadData.val.map((v, i) => v - data.hadData.error_lower[i] || 0),
    );
    ...
    Plotly.newPlot(
      "forcings-chart",
      [traceCO2, traceOther, traceTotal, traceObs, traceAnt],
      layout,
      config,
    );
  }
